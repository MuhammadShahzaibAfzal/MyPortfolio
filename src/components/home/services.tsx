      ...
      <div className="container">
        ...
        <div className="flex w-full flex-col lg:flex-row  gap-10 my-20">
          {services.map((service, index) => {
            return (
              <div
                className="border-[0.5px] flex-1 border-gray-400 rounded-xl p-6 cursor-pointer hover:bg-lightHover hover:-translate-y-1 transition-all duration-300 hover:shadow-black"
                key={index}
              >
                <div className="h-14 w-14 rounded-md bg-pink-500 flex justify-center items-center">
                  {<service.icon className="size-7 text-white" />}
                </div>
                <h3 className="my-4 font-semibold text-gray-700">{service.title}</h3>
